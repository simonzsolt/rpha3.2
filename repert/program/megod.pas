...
begin
  ...
  if p+e+r+m+v+k+u+s+x>1 then begin
    ...
    if k>0 then begin s2:=s2|' '|encint(k,1)|' catholique';
                   if k>1 then s2:=s2|'s';
    ...
    if e>0 then begin s2:=s2|' '|encint(e,1)|' luth‚rienne';
                   if e>1 then s2:=s2|'s';
                   if i>1 then begin s2:=s2|','; i:=i-1; end;
                 end;
    if r>0 then begin s2:=s2|' '|encint(r,1)|' calviniste';
                   if r>1 then s2:=s2|'s';
                   if i>1 then begin s2:=s2|','; i:=i-1; end;
                 end;
    if u>0 then begin s2:=s2|' '|encint(u,1)|' unitaire';
                   if u>1 then s2:=s2|'s';
                   if i>1 then begin s2:=s2|','; i:=i-1; end;
                 end;
    if s>0 then begin s2:=s2|' '|encint(s,1)|' juda‹sante';
                   if s>1 then s2:=s2|'s';
                   if i>1 then begin s2:=s2|','; i:=i-1; end;
                 end;
